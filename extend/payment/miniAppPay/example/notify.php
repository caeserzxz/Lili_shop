...
class PayNotifyCallBack extends WxPayNotify
{
	...
	{
        trace('1NotifyProcess:'.json_encode($data),'debug');
        ...
        if (!$this->Queryorder($data["transaction_id"])) {
            $msg = "订单查询失败";
            //Log::DEBUG("call back:" . $msg);
            trace('2NotifyProcess:'.json_encode($data),'debug');
            return false;
        }
        ...
        if (stripos($order_sn, 'recharge') !== false) {
            ...
            if ((string)($orderInfo['order_amount'] * 100) != (string)$data['total_fee']) {
                trace('3NotifyProcess:'.json_encode($orderInfo),'debug');//recharge157414188434
                return false; //验证失败
            }
